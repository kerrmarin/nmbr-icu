...
# where the icu configuration script is
ICU_main_configure := $(ICU_root)/source/runConfigureICU
...
$(HOST_dir):
	...
	$(ICU_main_configure) MacOSX --prefix=$(ICU_destination) $${OTHER_CONFIG} ; \
	$(MAKE_PARALLEL) ; \
...
$(ICU_clone):
	mkdir -p $@ && cd $@; \
	git clone --depth 1 --branch $(ICU_tag) https://github.com/unicode-org/icu.git
